...
async fn test_e2e_http_scan_workflow() {
    ...
    let report = engine.scan(&Target::Url(url.clone())).await;
...
async fn test_e2e_static_analysis_workflow() {
    ...
    let report = engine.scan(&Target::Path(temp_dir.path().to_path_buf())).await;
...
async fn test_invalid_url_handling() {
    ...
    let result = engine.scan(&Target::Url("not-a-valid-url".to_string())).await;
...
fn test_scanner_config_combinations() {
    ...
    // All flags enabled
    let all_enabled = ScannerConfig::new()
        .with_aggressive(true)
        .with_ddos(true)
        .with_stress(true);
    assert!(all_enabled.aggressive);
    assert!(all_enabled.ddos);
    assert!(all_enabled.stress);
}
